...
$(TEST_PATH)/%.s:$(TEST_PATH)/%.sy
	@timeout 60s $(BINARY) $< -o $@ -S 2>$(addsuffix .log, $(basename $@))
...
test:app
	...
	do
		...
		if [ $$? != 0 ]; then
			...
			if [ -f "$${IN}" ]; then
				timeout 30s qemu-arm -L /usr/arm-linux-gnueabihf $${BIN} <$${IN} >$${RES} 2>>$${LOG}
			else
				timeout 30s qemu-arm -L /usr/arm-linux-gnueabihf $${BIN} >$${RES} 2>>$${LOG}
			fi
			RETURN_VALUE=$$?
			FINAL=`tail -c 1 $${RES}`
